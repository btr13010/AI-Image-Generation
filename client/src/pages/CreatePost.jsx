  ...
  const generateImage = async () => {
		if (form.prompt) {
			try {
				...
				const response = await fetch('https://dall-e-wauu.onrender.com/api/v1/dalle', {
					method: 'POST',
					headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        ...
        // Go back to home page after posting the image
        navigate('/');
      } catch (error) {
        alert(error);
      } finally {
    ...
    <section className='max-w-7xl mx-auto'>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          {/* Input prompt field, along with a surprise me button*/}
          <FormField 
            labelName = "Prompt"
            type = "text"
            name = "prompt"
